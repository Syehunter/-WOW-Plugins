...
function Objectives:_SetState(block, plan)
    ...
    if text then
        self:AddObjective(block, 'BossKill', text, nil, nil, true, color)
    end
end
...
function Objectives:Layout()
    ...
    for i, plan in ipairs(Profile:GetTrackList()) do
        ...
        if item:GetProgressObject() then
            self:AddProgressBar(block, self:AddObjective(block, 'ProgressBar', item:GetProgressName(), nil, nil, true), item:GetProgressRate() * 100, item:GetProgressColor())
        end
