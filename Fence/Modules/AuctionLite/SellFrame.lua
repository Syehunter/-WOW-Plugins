...
function AuctionLite:UserChangedStacks()
  ...
  if ChangedSize <= 0 then
	...
	stacks = 1
	size = 1
	SellStacks:SetNumber(1)
	SellSize:SetNumber(1)
    end
...
function AuctionLite:ClickAuctionSellItemButton_Hook()
  ...
  if AuctionFrameSell:IsShown() and not self:CreateInProgress() then
    ...
    if name ~= nil then
      local _, _, _, _, _, _, _, maxSize = GetItemInfo(link);
	if (not maxSize) then maxSize = 1 end
...
dwStaticPopupDialogs["AL_VENDOR_WARNING"] = {
...
function AuctionLite:SellCreateAuctionButton_OnClick()
  ...
  if 0 < buyout and buyout <= (vendor * size / count) then
    dwStaticPopup_Show("AL_VENDOR_WARNING");
  else
...
function SellDurationDropDown_OnClick(self)
	AuctionFrameAuctions.duration = self.value;
	AuctionLite.db.profile.duration = self.value;
	UIDropDownMenu_SetSelectedValue(SellDurationDropDown, self.value);
	UIDropDownMenu_SetSelectedValue(DurationDropDown, self.value);
	UpdateDeposit();
	AuctionLite:UpdateDeposit();	
end

function SellDurationDropDown_Initialize()
	local info = UIDropDownMenu_CreateInfo();

	info.text = AUCTION_DURATION_ONE;
	info.value = 1;
	info.checked = nil;
	info.func = SellDurationDropDown_OnClick;
...
function SellDurationDropDown_OnShow(self)
	UIDropDownMenu_Initialize(self, SellDurationDropDown_Initialize);
	if ( not AuctionLite.db.profile.duration ) then
		AuctionLite.db.profile.duration = 2;
	end
	UIDropDownMenu_SetSelectedValue(SellDurationDropDown, AuctionLite.db.profile.duration);
end

function HookDurationDropDown_OnShow()
	hooksecurefunc("DurationDropDown_OnShow", function(self)
		if ( not AuctionLite.db.profile.duration ) then
			AuctionLite.db.profile.duration = 2;
		end
		UIDropDownMenu_SetSelectedValue(DurationDropDown, AuctionLite.db.profile.duration);
	end);
end

function HookDurationDropDown_OnClick()
	hooksecurefunc("DurationDropDown_OnClick", function(self)
		AuctionLite.db.profile.duration = self.value;
	end);
end

function HookAuctionsFrameAuctions_ValidateAuction()
	hooksecurefunc("AuctionsFrameAuctions_ValidateAuction", function(self)
		if ( not AuctionLite.db.profile.duration ) then
			AuctionLite.db.profile.duration = 2;
		end
		UIDropDownMenu_SetSelectedValue(DurationDropDown, AuctionLite.db.profile.duration);
...
dwAsynCall("Blizzard_AuctionUI", "HookDurationDropDown_OnShow");
dwAsynCall("Blizzard_AuctionUI", "HookDurationDropDown_OnClick");
dwAsynCall("Blizzard_AuctionUI", "HookAuctionsFrameAuctions_ValidateAuction");

-- Get the auction duration.
function AuctionLite:GetDuration()
  local time = UIDropDownMenu_GetSelectedValue(SellDurationDropDown) or 0;
  return time;
end
