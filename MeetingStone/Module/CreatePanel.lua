...
function CreatePanel:OnInitialize()
    ...
    local MinLevel = GUI:GetClass('NumericBox'):New(VoiceItemLevelWidget) do
        ...
        MinLevel:SetMinMaxValues(0, 100)
    ...
    -- local EditButton = CreateFrame('Button', nil, ViewBoardWidget, 'UIPanelButtonTemplate') do
    --     EditButton:SetPoint('RIGHT', DisbandButton, 'LEFT')
    --     EditButton:SetSize(120, 22)
    --     EditButton:SetText(L['编辑活动'])
    --     EditButton:SetScript('OnClick', function(CreateButton)
    --         self:ShowViewboard(false)
    --     end)
    --     MagicButton_OnLoad(EditButton)
    -- end
...
function CreatePanel:UpdateControlState()
    ...
    local pvp = IsHasPVPRating(activityItem and activityItem.activityId)
    ...
    self.PvPRating:SetEnabled(editable and pvp)

    self.DisbandButton:SetEnabled(isCreated and isLeader)
    self.CreateButton:SetEnabled(enable and isLeader)

    if enable then
        self.ItemLevel:SetMinMaxValues(0, GetPlayerItemLevel(IsPvPActivity(activityItem.activityId)))
...
function CreatePanel:InitProfile()
    ...
    if IsSoloCustomID(customId) then
        iLvl = min(100, GetPlayerItemLevel(IsPvPActivity(activityId)))
...
function CreatePanel:PARTY_LEADER_CHANGED()
    ...
    activity:SetItemLevel(min(activity:GetItemLevel(), GetPlayerItemLevel(activity:IsPvPActivity())))
