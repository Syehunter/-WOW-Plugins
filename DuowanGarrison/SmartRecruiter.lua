...
elseif GetLocale() == "koKR" then
	L.TextShift = "쉬프트키를 눌러 다른 능력들 보기..."
	L.TextAlt = "알트키를 눌러 능력 조합 보기..."
	L.ShiftDown = "다른 능력들"
	L.AltDown = "능력 조합"
	L.AbilityCountFormat = "%s(%d/%d)"
...
local function FollowerTooltip_HideCondition()
	if bMasterPlanLoaded == false then
		bMasterPlanLoaded = IsAddOnLoaded("MasterPlan") --by eui.cc
		if not bMasterPlanLoaded then
			bMasterPlanLoaded = IsAddOnLoaded("EuiGarrison")
		end
	end
...
local function FollowerButton_OnEnter(self)
	...
	GarrisonFollowerTooltip_Show(info.garrFollowerID or info.followerID, collected, info.quality, info.level, info.xp, info.levelXP, info.iLevel, abilities[1], abilities[2], abilities[3], abilities[4], traits[1], traits[2], traits[3], traits[4], false)
end
