...
local f = CreateFrame('Frame', 'DWWorldFlightMapFrame', WorldMapButton)
f:SetAllPoints()
...
local function TaxiNodeOnButtonEnter(button) 
	...
	if ( type == "REACHABLE" ) then
		SetTooltipMoney(WorldMapTooltip, TaxiNodeCost(button:GetID()));
...
function DWWorldFlightMap_Toggle(toggle)
	...
	if toggle == 1 then
		...
		DWWorldFlightMapFrame:RegisterEvent('TAXIMAP_OPENED')
		DWWorldFlightMapFrame:RegisterEvent('TAXIMAP_CLOSED')
	else
		DWWorldFlightMapFrame:UnregisterEvent('TAXIMAP_OPENED')
		DWWorldFlightMapFrame:UnregisterEvent('TAXIMAP_CLOSED')
		TaxiFrame:RegisterEvent("TAXIMAP_OPENED");
		TaxiFrame:RegisterEvent("TAXIMAP_CLOSED");		
	end
end

DWWorldFlightMap_Toggle(dwGetCVar('MiniMapMod','WorldFlightMapEnable') or 1)