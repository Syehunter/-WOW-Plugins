...
function panel:OnShow ( ... )
	...
	if isWindowed and not WorldMapMinimized then
		...
		_NPCScanOverlayKey:SetPoint("BOTTOMRIGHT", WorldMapButton, "BOTTOMLEFT")
	elseif not isWindowed and WorldMapMinimized then
		_NPCScanOverlayKey:ClearAllPoints()
		_NPCScanOverlayKey:SetPoint( Points[ Point % #Points + 1 ], WorldMapButton )
...
do
	...
	function panel:KeyOnEnter ()
		if (private.Options.LockSwap and not IsShiftKeyDown()) or (not private.Options.LockSwap and IsShiftKeyDown() )then
			self:ClearAllPoints()
			self:SetPoint( Points[ Point % #Points + 1 ], WorldMapButton )
			Point = Point + 1
		end
	end
end
...
function panel:OnLoad ( ... )
	...
	local KeyParent = CreateFrame( "Frame", "_NPCScanOverlayKeyParent", WorldMapFrame )
