...
WorldMap = Addon:NewModule(Addon:GetClass('Map'):New(WorldMapButton), 'WorldMap', 'AceEvent-3.0', 'AceHook-3.0')
...
function WorldMap:OnInitialize()
    ...
    local Button = CreateFrame('Button', nil, WorldMapFrame.UIElementsFrame) do
        ...
        local Icon = Button:CreateTexture(nil, 'ARTWORK') do
            ...
            SetPortraitToTexture(Icon, [[Interface\AddOns\Collector\Media\Logo]])
        end
...
function WorldMap:Draw()
    local function draw(_, obj)
        if obj:HasNotCollectedItem(self.klasses) then
            self:DrawObject(obj, self.area)
        end
    end

    InvokeForEachi(self.area:GetNpcList(), draw)
    InvokeForEachi(self.area:GetGameObjectList(), draw)
