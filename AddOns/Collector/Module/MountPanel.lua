...
MountPanel = Addon:NewModule(CreateFrame('Frame', nil, MountJournal), 'MountPanel', 'AceEvent-3.0', 'AceHook-3.0', 'AceTimer-3.0')

function MountPanel:OnInitialize()
...
function MountPanel:InitHook()
    ...
    -- self:SecureHook(MountJournal, 'SetAlpha', function(_, alpha)
...
function MountPanel:MountJournal_Select(index)
    C_Timer.After(0, function()
        local visibleIndex = tIndexOf(MountJournal.cachedMounts, index)
...
function MountPanel:MountJournal_Pickup(index)
...
function MountPanel:COLLECTOR_FIRST_LOGIN()
...
function MountPanel:COLLECTOR_PLANLIST_UPDATE()
    self:UpdateDisplay()
    self:Refresh()
end

function MountPanel:ResetFilterAndSort()
...
function MountPanel:SetTypeFilter(filter, id, flag)
    self.filterTypes[filter][id] = not flag or nil
    self:Refresh()
end

function MountPanel:IsTypeFiltered(filter, id)
    return not self.filterTypes[filter][id]
end

function MountPanel:AddAllTypeFilters(filter)
    wipe(self.filterTypes[filter])
    self:Refresh()
end

function MountPanel:ClearAllTypeFilters(filter)
    local data = MOUNT_JOURNAL_FILTER_TYPES[filter]
...
function MountPanel:IsAnyTypeFiltered(filter)
    local data = MOUNT_JOURNAL_FILTER_TYPES[filter]
    for _, v in ipairs(data) do
...
function MountPanel:IsAnyTypeNotFiltered(filter)
    return next(self.filterTypes[filter])
end

function MountPanel:SetFavoriteAtTop(flag)
    self.favoriteAtTop = flag or nil
    self:Refresh()
end

function MountPanel:IsFavoriteAtTop()
    return self.favoriteAtTop
end

function MountPanel:SetPlanAtTop(flag)
    self.planAtTop = flag or nil
    self:Refresh()
end

function MountPanel:IsPlanAtTop()
    return self.planAtTop
end

function MountPanel:SetNewAtTop(flag)
    self.newAtTop = flag or nil
    self:Refresh()
end

function MountPanel:IsNewAtTop()
    return self.newAtTop
end

function MountPanel:SetOnlyCurrentArea(flag)
    self.onlyCurrentArea = flag
    self:Refresh()
end

function MountPanel:IsOnlyCurrentArea()
    return self.onlyCurrentArea
end

function MountPanel:SetSortKey(key)
    self.sortKey = key
    self:Refresh()
end

function MountPanel:GetSortKey()
    return self.sortKey
end

function MountPanel:GetSearchString()
    return MountJournal.searchString
end
...
function MountPanel:MakeSortValue(mount, isCollected, isFavorite, inPlan)
    local key1 do
...
function MountPanel:MatchMount(mount, isCollected, isFavorite, inPlan)
...
function MountPanel:UpdateCache(frame)
...
function MountPanel:UpdateDisplay()
...
function MountPanel:ToggleMountDropDown(index, owner)
...
function MountPanel:GetMountMenuTable(mount, isCollected, isActive)
    local menuTable = {}
    ...
    while true do
        ...
        if m == nil or m > 0 then
            sort(Addon.mountCache, function(a, b)
                a = a:GetAttribute(key) or 0
                b = b:GetAttribute(key) or 0
            ...
            for i = 1, m or #Addon.mountCache do
                ...
                if v and v:GetAttribute('Faction') ~= NOT_PLAYER_FACTION and not v:IsCollected() and not v:IsInPlan() and v:IsValid() and not GET_MOUNT_CACHE[v] then
