...
--Quester 6.2.0.1
--任务等级显示，任务难度着色，任务完成音效
local L = LibStub("AceLocale-3.0"):GetLocale("Quester")
local QuestTitle = GetAddOnMetadata("DuowanQuest", "Title");
...
function Quester:OnInitialize()
	...
	LibStub("AceConfigRegistry-3.0"):RegisterOptionsTable(QuestTitle, getOptionsTable)
	local optFrame = LibStub("AceConfigDialog-3.0"):AddToBlizOptions(QuestTitle, QuestTitle)
	self.optFrame = optFrame;

	self:RegisterChatCommand("dwquester", function() InterfaceOptionsFrame_OpenToCategory(optFrame) end)
