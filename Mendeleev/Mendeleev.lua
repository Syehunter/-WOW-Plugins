...
-- 11th item:id field and level adjustment
local levelAdjust={ 
...
function Mendeleev:OnTooltipSetItem(tooltip, ...)
	...
	if iLevel and db.showItemLevel then			
		local iAjustLevel = ItemUpgradeInfo:GetUpgradedItemLevel(item)
		tooltip:AddDoubleLine(L["iLevel"], iAjustLevel)
	end

	-----------------
	-- 显示幻化套装信息
	if (db.showHuahuaSet) then
		local iType, subType, setName, setsdata = huanhuadata:FindItem(itemID);		
		if (iType) then
