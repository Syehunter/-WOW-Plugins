...
function TradeskillInfo:UpdateSkills()
	...
	if not self.vars.skillnames then
		TradeskillInfo:PopulateProfessionNames()
	end
	self.vars.playername = UnitName("player")
	if not self.db.realm[self.vars.playername] then
		self.db.realm[self.vars.playername] = {}
	end
